package dev.lightdream.ultraprisoncore.friends.gui;

import dev.lightdream.ultraprisoncore.friends.UltraPrisonFriends;
import dev.lightdream.ultraprisoncore.utils.Item;
import dev.lightdream.ultraprisoncore.UltraPrisonCore;
import dev.lightdream.ultraprisoncore.utils.ItemStackUtils;
import dev.lightdream.ultraprisoncore.utils.Utils;
...
public class OnlinePlayersGUI implements GUI {
    ...
    public OnlinePlayersGUI(UltraPrisonCore core, UltraPrisonFriends module, UUID player, int page) {
        this.core = core;
        this.module = module;
        this.player = player;
        this.page = page;
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        if (guiUse != null) {
            switch ((String) guiUse) {
                case "back":
                    player.openInventory(new OnlinePlayersGUI(core, module, this.player, page - 1).getInventory());
                    break;
                case "next":
                    player.openInventory(new OnlinePlayersGUI(core, module, this.player, page + 1).getInventory());
                    break;
                case "player":
                    ...
                    switch (event.getClick()) {
                        ...
                        case MIDDLE:
                            ...
                            break;
                        case LEFT:
                            module.getFriendsManager().addFriend(player, friend);
                            break;
                    }
                    break;
                case "open_friends_gui":
                    player.openInventory(new FriendsGUI(core, module, this.player, 0).getInventory());
                    break;
            }
        }
    ...
    public Inventory getInventory() {
        ...
        inventory.setItem(module.getFriendsGUIConfig().openFriendsGUI.slot, Utils.setNBT(Utils.setNBT(ItemStackUtils.makeItem(module.getFriendsGUIConfig().openFriendsGUI), "gui_use", "open_friends_gui"), "gui_protect", true));
        ...
        Item playerTemplate = module.getFriendsGUIConfig().playerItem;
